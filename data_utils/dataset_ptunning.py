...
def load_au(filename,q):
    data = []
    dic_text = {}
    with open(filename+'texts_raw_fixed.txt','r') as text_data:
        lines = text_data.readlines()
        for line in lines:
            line = line.split('\t')
            dic_text[line[0]] = line[1]
    with open(filename+'train_random.txt','r') as train_data:
        lines = train_data.readlines()
        for line in lines:
            line = line.split('\t')
            dic_temp = {}
            dic_temp['q1'] = dic_text[line[0]]
            dic_temp['q2'] = dic_text[line[1].split(' ')[0]]
            dic_temp['q'] = q
            dic_temp['label'] = 'relevant'
            data.append(dic_temp)
            dic_temp = {}
            dic_temp['q1'] = dic_text[line[0]]
            dic_temp['q2'] = dic_text[line[2].split(' ')[0]]
            dic_temp['q'] = q
            dic_temp['label'] = 'irrelevant'
            data.append(dic_temp)
    return data
def load_paws(filname,q):
    data = []
...
def load_json_trivia(filname,q):
    data = []
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
        for index in range(len(dic)-1):
...
def load_json_trivia_ndcg(filname,q):
    data = []
    dic_qels_dev = {}
    dic_qels_test = {}
    with open(filname, 'r') as load_f:
...
def load_json_squad_all(filname,q):
    data = []
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
        for index in range(len(dic)-1):
...
def load_json_trivia_all(filname,q):
    data = []
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
        for index in range(len(dic)-1):
...
def load_json_trivia_mrr(filname,q):
    data = []
    truth_dic = {}
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
...
def load_json_trivia_mrr_few(filname,q):
    data = []
    truth_dic = {}
    count = 0
    with open(filname, 'r') as load_f:
...
def load_json_trivia_mrr_task(filname,q):
    data = []
    truth_dic = {}
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
...
def load_json_nq_mrr(filname,q):
    data = []
    truth_dic = {}
    count = 0
    with open(filname, 'r') as load_f:
...
def load_dialogue_mrr(filname,q):
    data = []
    pos = 0
    neg = 0
    truth_dic ={}
...
def load_dialogue_mrr_task(filname,q):
    data = []
    pos = 0
    neg = 0
    truth_dic ={}
...
import re
def load_dailydialogue_mrr(filname,answer_base,q):
    data = []
    truch_dic = {}
    q1_num = 0
    with open(filname,'r') as reddit:
...
def load_dailydialogue_mrr_task(filname,answer_base,q):
    data = []
    truch_dic = {}
    q1_num = 0
    with open(filname,'r') as reddit:
...
def load_amazon_task(filname,q):
    data = []
    truth_dic = {}
    # with open(filname,'r') as amazon:
    #     dic = jsonlines.Reader(amazon)
...
def load_dia_movie(conversation,lines,q):
    data = []
    dic_line = {}
    ...
    with open(conversation,'r',encoding='utf-8') as conver:
        ...
        for line in lines:
            try:
                ...
                # print(dic_temp)
    #random.shuffle(data)
    return data[:5000],data[5000:],data[10000:]
...
class MultiDataset_yes_no(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return self.data[i]['q1'] + '[SEP]' + self.data[i]['q2'] + '[SEP]' + self.data[i]['q'], label
...
class MultiDataset_yes_no_ndcg(Dataset):
    ...
    def __getitem__(self, i):
        if (self.data[i]['label'] == 'relevant'):
            ...
            label = 'no'
        # str_pair = self.data[i]['q1_real'] + '[NDCG]' + self.data[i]['q2_real'] + '[NDCG]'
        return self.data[i]['q1_real'] + '[NDCG]' + self.data[i]['q2_real'] + '[NDCG]' +self.data[i]['q1'] + '[SEP]' + self.data[i]['q2'] + '[SEP]' + self.data[i]['q'], label
        # return str_pair + self.data[i]['q1'] + '[SEP]' + self.data[i]['q2'] + '[SEP]' + self.data[i]['q'], label
...
class MultiDataset_yes_no_mrr(Dataset):
    ...
    def __getitem__(self, i):
        if (self.data[i]['label'] == 'relevant'):
            ...
            label = 'no'
        # str_pair = self.data[i]['q1_real'] + '[NDCG]' + self.data[i]['q2_real'] + '[NDCG]'
        return self.data[i]['q1_num'] + '[MRR]' + self.data[i]['q2_num'] + '[MRR]' + self.data[i]['q1'] + '[SEP]' + self.data[i]['q2'] + '[SEP]' + self.data[i]['q'], label
