...
def load_msrp_mark(filname,mark):
    data = []
    pos = 0
    neg = 0
    with open(filname,'r') as dataset:
    ...
    with open(filname, 'r') as load_f:
        ...
        for index in range(len(dic)-1):
            ...
            if(len(dic[index]['negative_ctxs']) > 1):
                dic_temp = {}
                dic_temp['q1'] = dic[index]['question']
                dic_temp['q2'] = dic[index]['negative_ctxs'][1]['text']
                dic_temp['label'] = 'irrelevant'
                data.append(dic_temp)
    return data
...
def load_json_trivia_mark(filname,mark):
    data = []
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
        for index in range(len(dic)-1):
    ...
    for index in range(len(data)):
        data[index]['q1'] = mark + data[index]['q1']
    return data
...
def load_json_squad_all(filname):
    data = []
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
        for index in range(len(dic)-1):
...
def load_json_trivia_all(filname):
    data = []
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
        for index in range(len(dic)-1):
...
def load_json_trivia_mrr(filname):
    data = []
    truch_dic = {}
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
        for index in range(len(dic)-1):
            ...
            for hard_neg_idx in range(len(dic[index]['hard_negative_ctxs'])):
                ...
                count_index += 1
                data.append(dic_temp)
            truch_dic[str(index)] = truch_list
    return data,truch_dic


def load_json_trivia_mrr_task(filname):
    data = []
    truth_dic = {}
    with open(filname, 'r') as load_f:
        dic = json.load(load_f)
...
def load_json_trivia_mrr_few(filname):
    data = []
    truch_dic = {}
    count = 0
    with open(filname, 'r') as load_f:
...
# def load_adhoc_mq_train(filname):
#     train_data = []
#     dev_data = []
#     test_data = []
#     dic_dataset = {}
...
def load_adhoc_mq_train_mark(filname,mark):
    train_data = []
    dev_data = []
    test_data = []
    dic_dataset = {}
    ...
    for index in range(len(train_data)):
        train_data[index]['q1'] = mark + train_data[index]['q1']

    for index in range(len(dev_data)):
        dev_data[index]['q1'] = mark + dev_data[index]['q1']

    for index in range(len(test_data)):
        test_data[index]['q1'] = mark + test_data[index]['q1']
    #shuffle 防止正负样本各自堆在一起 虽然可能截取到的测试集和验证集数据不同，不过影响不大
    return train_data[:20000],dev_data[:5000],test_data[:5000]
...
def load_mli_mark(filname,mark):
    data = []
    with open(filname,'r') as mli:
        json_data = json.load(mli)
        for item in json_data:
...
def load_dialogue_mrr_task(filname):
    data = []
    pos = 0
    neg = 0
    truth_dic ={}
...
def load_dailydialogue_mrr_task(filname,answer_base):
    data = []
    truch_dic = {}
    q1_num = 0
    with open(filname,'r') as reddit:
...
def load_dailydialogue_mark(filname,mark):
    data = []
    with open(filname,'r') as reddit:
        lines = reddit.readlines()
        last_q2 = ''
        ...
        for idx in range(len(lines)-1): ##确定这一行是对话轮数最多的那行
            if(not '__eou__' in lines[idx+1]):
                ...
                for idx_line in range(1,len(line)):
                    ...
                    neg_num = 0
                    while 1:
                        if (not '__eou__' in lines[neg_index]): ## 循环寻找满足条件的负样本，每个query找9个
                            neg_index = (neg_index+1) % len_all
                            continue
                        #neg_line = (lines[neg_index].split('__eou__'))[0]
...
import re
def load_dailydialogue_mrr(filname,answer_base):
    data = []
    truch_dic = {}
    q1_num = 0
    with open(filname,'r') as reddit:
...
def load_amazon_task(filname):
    ...
    for item in dic:
        ...
        dic_temp = {}
        dic_temp['q1'] = item['questionText']
        dic_temp['q2'] = item['answers'][0]['answerText']
        dic_temp['q1_num'] = str(dic.index(item))
        dic_temp['q2_num'] = str(0)
        dic_temp['label'] = 'relevant'
        truth_dic[dic_temp['q1_num']].append(dic_temp['q2_num'])
...
class NQDataset_fintune_mrr(Dataset):
    def __init__(self, dataset_type, data, tokenizer, args):
        super().__init__()
        self.args = args
        self.data = data
    ...
    def __getitem__(self, i):
        ...
        if(self.data[i]['label'] == 'relevant'):
            ...
            label = 0
        return self.data[i]['q1_num'] + '[MRR]' + self.data[i]['q2_num'] + '[MRR]' +self.data[i]['q1'] + '[SEP]' + self.data[i]['q2'], label

class NQDataset_fintune_mrr_yes_no(Dataset):
    def __init__(self, dataset_type, data, tokenizer, args):
        super().__init__()
        self.args = args
        self.data = data
...
class NQDataset_fintune_mrr_mtdnn(Dataset):
    def __init__(self, dataset_type, data, tokenizer, args,task_tag):
        super().__init__()
        self.args = args
        self.data = data
        self.dataset_type = dataset_type
    ...
    def __getitem__(self, i):
        ...
        if(self.data[i]['label'] == 'relevant'):
            ...
            label = 0
        return self.task_tag + self.data[i]['q1_num'] + '[MRR]' + self.data[i]['q2_num'] + '[MRR]' +self.data[i]['q1'] + '[SEP]' + self.data[i]['q2'], label

class NQDataset_fintune_ndcg(Dataset):
    def __init__(self, dataset_type, data, tokenizer, args):
        super().__init__()
        self.args = args
        self.data = data
    ...
    def __getitem__(self, i):
        ...
        if(self.data[i]['label'] == 'relevant'):
            ...
            label = 0
        return self.data[i]['q1_real'] + '[NDCG]' + self.data[i]['q2_real'] + '[NDCG]' +self.data[i]['q1'] + '[SEP]' + self.data[i]['q2'], label

class NQDataset_fintune_ndcg_yes_no(Dataset):
    def __init__(self, dataset_type, data, tokenizer, args):
        super().__init__()
        self.args = args
        self.data = data
...
class NQDataset_fintune_ndcg_mtdnn(Dataset):
    def __init__(self, dataset_type, data, tokenizer, args,task_type):
        super().__init__()
        self.args = args
        self.data = data
        self.dataset_type = dataset_type
    ...
    def __getitem__(self, i):
        ...
        if(self.data[i]['label'] == 'relevant'):
            ...
            label = 0
        return self.task_type + self.data[i]['q1_real'] + '[NDCG]' + self.data[i]['q2_real'] + '[NDCG]' +self.data[i]['q1'] + '[SEP]' + self.data[i]['q2'], label
...
class DiaDataset_fintune_yes_no(Dataset):
    def __init__(self, dataset_type, data):
        super().__init__()
        self.data = data
        self.dataset_type = dataset_type
...
class DiaDataset_fintune_yes_no_new(Dataset):
    def __init__(self, dataset_type, data):
        super().__init__()
        self.data = data
        self.dataset_type = dataset_type
    ...
    def __getitem__(self, i):
        ...
        if(self.data[i]['label'] == 'relevant'):
            ...
            label = 'No.'
        return self.data[i]['q1'], label


class DiaDataset_fintune_yes_no_task(Dataset):
    def __init__(self, dataset_type, data,task):
        super().__init__()
        self.data = data
        self.dataset_type = dataset_type
        self.task = task
