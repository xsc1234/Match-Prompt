...
def load_adhoc_mq(filname):
    ...
    print(test_neg)
    random.shuffle(train_neg_list)
    train_neg_list = train_neg_list[:3*train_pos]
    random.shuffle(dev_pos_list)
    dev_neg_list = dev_neg_list[:dev_pos]
    random.shuffle(test_pos_list)
    test_neg_list = test_neg_list[:test_pos]
    ...
    return (train_data+test_data)[:40000],dev_data[:5000],dev_data[:5000]




def load_adhoc_mq07(filname):
    train_data = []
    dev_data = []
    test_data = []
    dic_dataset = {}
    ...
    return train_data[:40000],dev_data[:5000],dev_data[:5000]
...
class MultiDataset_t5(Dataset):
    ...
    def __getitem__(self, i):
        if (self.data[i]['label'] == 'relevant'):
            ...
            label = 'No.'
        return 'Text1:' + self.data[i]['q1'] + 'Text2:' + self.data[i]['q2'], label
